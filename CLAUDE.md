...
accounts for 2016/2019/2022/2025, built from a hand-curated Excel workbook
(`Brasil_2016_2019_2022_2025.xlsx`). No framework, no build step, no package manager —
plain HTML/CSS/JS served from `docs/`, plus a Python script that flattens the workbook
into a JS data file. Charts via Chart.js (CDN). See `README.md` for the user-facing
feature list (pages, unit toggle, dark mode, CSV export, mobile drawer menu).
...
**Data pipeline:** `Brasil_2016_2019_2022_2025.xlsx` (one sheet per theme, Portuguese
column headers, values for 2016/2019/2022/2025) → `scripts/gerar_dados.py` → single
global `window.DADOS = {...}` written to `docs/data/dados.js`. `DADOS.abas.<chave>.linhas`
is a flat list of rows per sheet, each with `cod`, `nome`, `estagio` (budget stage or
concept), `unidade`, `v[4]` (raw values by year), `fontes`, `classe`, `obs`. `ABAS` in
`gerar_dados.py` maps the internal key (`"pib"`, `"receitas"`, `"afirmativas"`, …) to the
sheet's real name — that mapping is the source of truth for which sheet backs which key.
`CORRECOES` in the same file patches two known data-entry bugs in the source sheet
(see README's "Correções em relação à planilha" — don't re-introduce them if the
workbook is regenerated upstream).

**Script load order matters** (see `docs/index.html`): `dados.js` → `paginas.js` →
`avaliacoes.js` → `leituras.js` → `checagem.js` → `app.js`. Each of the first four just
assigns a `window.*` global inside an IIFE; `app.js` is the only one with behavior and
expects all the others to already exist on `window`.

**Content model — four parallel files keyed by page id**, merged at render time:
...
- `docs/js/avaliacoes.js` (`window.AVALIACOES[id]`), `docs/js/leituras.js`
  (`window.LEITURAS[id]`), `docs/js/checagem.js` (`window.CHECAGEM[id]`) — optional
  editorial content per page (impact assessment cards, "liberal vs. desenvolvimentista"
  framings, and political-narrative fact-checks with an international-reference-country
  mirror). `app.js`'s `renderiza()` splices a synthetic level into `pg.niveis` for
  whichever of these three exist for that page id, in a fixed order (avaliação → duas
  leituras → narrativas × dados), each positioned right before the "tabela completa"
  level if there is one (the "visão geral" page is special-cased to slot avaliação in
  right after its first level instead, since it has no tabela level). To add any of
  these three blocks to a new page, just add an entry under that page's id in the
  relevant file — no change to `app.js` needed.
...
the "Educação" page, which has no dedicated sheet of its own — it pulls rows out of
`social`, `despesas`, and `afirmativas`). `resolveTabela()` in `app.js` normalizes both
forms.
...
**Root-level clutter:** besides the live pipeline (`Brasil_2016_2019_2022_2025.xlsx`,
`scripts/gerar_dados.py`, `docs/`), the repo root has several one-off/legacy scripts and
scraped source files from building and extending the workbook itself (`build.py`,
`build4.py`, `build_2016_2019_2022_2025.py`, `aff_block.py`, `patch_aff.py`,
`patch_edu.py`, `sintese*.txt`) plus an in-progress, not-yet-integrated companion
workbook effort (`build_quantitativo.py`, `Relatorio_quantitativo_2016_2019_2022_2025.xlsx`,
`fundeb*.html/.xls`, `cosems_sus.txt`, `ipea_fns2016.txt`, `search_fundeb2019.html`).
None of these are read by `scripts/gerar_dados.py` or referenced by the site — they're
provenance/history for the main workbook and raw material for a possible future data
source, not part of the active build.
